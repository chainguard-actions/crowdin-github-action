...
if [ -n "$INPUT_COMMAND" ]; then
  ...
  # Sanitize output: remove newlines that could inject extra key=value pairs
  SAFE_CROWDIN_OUTPUT=$(printf '%s' "$CROWDIN_OUTPUT" | tr -d '\r')

  # Write multiline output to GITHUB_OUTPUT using heredoc delimiter
  printf 'command_output<<%s\n%s\n%s\n' "CROWDIN_EOF" "$SAFE_CROWDIN_OUTPUT" "CROWDIN_EOF" >> "$GITHUB_OUTPUT"
