...
upload_sources() {
  ...
  echo "UPLOAD SOURCES"
  upload_sources_opts=()
  if [ -n "$UPLOAD_SOURCES_OPTIONS" ]; then
    while IFS= read -r -d '' t; do upload_sources_opts+=("$t"); done \
      < <(printf '%s' "$UPLOAD_SOURCES_OPTIONS" | xargs printf '%s\0')
  fi
...
upload_translations() {
  ...
  if [ -n "$UPLOAD_TRANSLATIONS_OPTIONS" ]; then
    while IFS= read -r -d '' t; do upload_translations_opts+=("$t"); done \
      < <(printf '%s' "$UPLOAD_TRANSLATIONS_OPTIONS" | xargs printf '%s\0')
  fi
...
download_sources() {
  ...
  if [ -n "$DOWNLOAD_SOURCES_OPTIONS" ]; then
    while IFS= read -r -d '' t; do download_sources_opts+=("$t"); done \
      < <(printf '%s' "$DOWNLOAD_SOURCES_OPTIONS" | xargs printf '%s\0')
  fi
...
download_translations() {
  ...
  if [ -n "$DOWNLOAD_TRANSLATIONS_OPTIONS" ]; then
    while IFS= read -r -d '' t; do download_translations_opts+=("$t"); done \
      < <(printf '%s' "$DOWNLOAD_TRANSLATIONS_OPTIONS" | xargs printf '%s\0')
  fi
...
create_pull_request() {
  ...
  if echo "$PULL_REQUESTS" | grep -xq "$BRANCH"; then
    ...
    if [ -n "$PULL_REQUESTS_URL" ]; then
      safe_pr_url=$(printf '%s' "$PULL_REQUESTS_URL" | tr -d '\n\r')
      echo "pull_request_url=$safe_pr_url" >> "$GITHUB_OUTPUT"
    fi

    if [ -n "$PULL_REQUESTS_NUMBER" ]; then
      safe_pr_number=$(printf '%s' "$PULL_REQUESTS_NUMBER" | tr -d '\n\r')
      echo "pull_request_number=$safe_pr_number" >> "$GITHUB_OUTPUT"
    fi
...
if [ -n "$INPUT_COMMAND_ARGS" ]; then
  command_args_tokens=()
  while IFS= read -r -d '' t; do command_args_tokens+=("$t"); done \
    < <(printf '%s' "$INPUT_COMMAND_ARGS" | xargs printf '%s\0')
  set -- "$@" "${command_args_tokens[@]}"
fi
...
if [ -n "$INPUT_COMMAND" ]; then
  ...
  # Tokenize INPUT_COMMAND_ARGS into an array for safe expansion
  crowdin_cmd_args=()
  if [ -n "$INPUT_COMMAND_ARGS" ]; then
    while IFS= read -r -d '' t; do crowdin_cmd_args+=("$t"); done \
      < <(printf '%s' "$INPUT_COMMAND_ARGS" | xargs printf '%s\0')
  fi

  # Capture command output while still displaying it
  CROWDIN_OUTPUT=$(crowdin "$INPUT_COMMAND" "${crowdin_cmd_args[@]}")
  echo "$CROWDIN_OUTPUT"

  # Write multiline output to GITHUB_OUTPUT using heredoc delimiter
  # Sanitize: strip any embedded CROWDIN_EOF lines to prevent delimiter injection
  safe_crowdin_output=$(printf '%s' "$CROWDIN_OUTPUT" | grep -v '^CROWDIN_EOF$')
  echo "command_output<<CROWDIN_EOF" >> "$GITHUB_OUTPUT"
  echo "$safe_crowdin_output" >> "$GITHUB_OUTPUT"
