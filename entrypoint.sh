...
if [ -n "$INPUT_COMMAND" ]; then
  cmd_tokens=()
  while IFS= read -r -d '' t; do cmd_tokens+=("$t"); done \
    < <(printf '%s' "$INPUT_COMMAND" | xargs printf '%s\0')
  cmd_args_tokens=()
  if [ -n "$INPUT_COMMAND_ARGS" ]; then
    while IFS= read -r -d '' t; do cmd_args_tokens+=("$t"); done \
      < <(printf '%s' "$INPUT_COMMAND_ARGS" | xargs printf '%s\0')
  fi
  ...
  CROWDIN_OUTPUT=$(crowdin "${cmd_tokens[@]}" "${cmd_args_tokens[@]}")
  echo "$CROWDIN_OUTPUT"

  # Write multiline output to GITHUB_OUTPUT using a random heredoc delimiter
  # to prevent injection via a crafted delimiter line in the output
...
if [ "$INPUT_DOWNLOAD_BUNDLE" ]; then
  ...
  bundle_id_tokens=()
  while IFS= read -r -d '' t; do bundle_id_tokens+=("$t"); done \
    < <(printf '%s' "$INPUT_DOWNLOAD_BUNDLE" | xargs printf '%s\0')
  crowdin bundle download "${bundle_id_tokens[@]}" "${DOWNLOAD_BUNDLE_ARGS[@]}"
