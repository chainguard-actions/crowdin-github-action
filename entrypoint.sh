...
upload_sources() {
  local _upload_sources_opts=()
  if [ -n "$INPUT_UPLOAD_SOURCES_ARGS" ]; then
    while IFS= read -r -d '' _t; do _upload_sources_opts+=("$_t"); done \
      < <(printf '%s' "$INPUT_UPLOAD_SOURCES_ARGS" | xargs printf '%s\0')
  fi

  echo "UPLOAD SOURCES"
  crowdin upload sources "$@" "${_upload_sources_opts[@]}"
}

upload_translations() {
  local _upload_translations_opts=()
  if [ -n "$INPUT_UPLOAD_LANGUAGE" ]; then
    _upload_translations_opts+=("--language=${INPUT_UPLOAD_LANGUAGE}")
  fi

  if [ "$INPUT_AUTO_APPROVE_IMPORTED" = true ]; then
    _upload_translations_opts+=(--auto-approve-imported)
  fi

  if [ "$INPUT_IMPORT_EQ_SUGGESTIONS" = true ]; then
    _upload_translations_opts+=(--import-eq-suggestions)
  fi

  if [ -n "$INPUT_UPLOAD_TRANSLATIONS_ARGS" ]; then
    while IFS= read -r -d '' _t; do _upload_translations_opts+=("$_t"); done \
      < <(printf '%s' "$INPUT_UPLOAD_TRANSLATIONS_ARGS" | xargs printf '%s\0')
  fi

  echo "UPLOAD TRANSLATIONS"
  crowdin upload translations "$@" "${_upload_translations_opts[@]}"
}

download_sources() {
  local _download_sources_opts=()
  if [ -n "$INPUT_DOWNLOAD_SOURCES_ARGS" ]; then
    while IFS= read -r -d '' _t; do _download_sources_opts+=("$_t"); done \
      < <(printf '%s' "$INPUT_DOWNLOAD_SOURCES_ARGS" | xargs printf '%s\0')
  fi

  echo "DOWNLOAD SOURCES"
  crowdin download sources "$@" "${_download_sources_opts[@]}"
}

download_translations() {
  local _download_translations_opts=()
  if [ -n "$INPUT_DOWNLOAD_LANGUAGE" ]; then
    _download_translations_opts+=("--language=${INPUT_DOWNLOAD_LANGUAGE}")
  elif [ -n "$INPUT_LANGUAGE" ]; then #back compatibility for older versions
    _download_translations_opts+=("--language=${INPUT_LANGUAGE}")
  fi

  if [ "$INPUT_SKIP_UNTRANSLATED_STRINGS" = true ]; then
    _download_translations_opts+=(--skip-untranslated-strings)
  fi

  if [ "$INPUT_SKIP_UNTRANSLATED_FILES" = true ]; then
    _download_translations_opts+=(--skip-untranslated-files)
  fi

  if [ "$INPUT_EXPORT_ONLY_APPROVED" = true ]; then
    _download_translations_opts+=(--export-only-approved)
  fi

  if [ -n "$INPUT_DOWNLOAD_TRANSLATIONS_ARGS" ]; then
    while IFS= read -r -d '' _t; do _download_translations_opts+=("$_t"); done \
      < <(printf '%s' "$INPUT_DOWNLOAD_TRANSLATIONS_ARGS" | xargs printf '%s\0')
  fi

  echo "DOWNLOAD TRANSLATIONS"
  crowdin download "$@" "${_download_translations_opts[@]}"
}
...
if [ -n "$INPUT_COMMAND_ARGS" ]; then
  _cmd_args=()
  while IFS= read -r -d '' _t; do _cmd_args+=("$_t"); done \
...
if [ -n "$INPUT_COMMAND" ]; then
  ...
  # Sanitize output to prevent newline injection into GITHUB_OUTPUT
